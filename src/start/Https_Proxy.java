...
public class Https_Proxy extends JPanel {
	private JTextField host;
	private JTextField port;
	private JTextField username;
	private JTextField password;

	public Https_Proxy(final JTabbedPane Pane, final JFrame frame) {
		...
		host.setText(Proxy.https_Host);
		port.setText(Proxy.https_Port);
		password.setText(Proxy.https_Password);
		username.setText(Proxy.https_Username);
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Pane.isEnabledAt(1)) {
					...
					Proxy.https_Host = host.getText();
					Proxy.https_Port = port.getText();
					Proxy.https_Password = password.getText();
					Proxy.https_Username = username.getText();
					frame.dispose();
				}
			}
		});
