...
public class SocksV5_Proxy extends JPanel {
	private JTextField host;
	private JTextField port;
	private JTextField username;
	private JTextField password;

	public SocksV5_Proxy(final JTabbedPane Pane, final JFrame frame) {
		...
		host.setText(Proxy.socks5_Host);
		port.setText(Proxy.socks5_Port);
		password.setText(Proxy.socks5_Password);
		username.setText(Proxy.socks5_Username);
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (Pane.isEnabledAt(3)) {
					new Proxy().useProxySocks_v5(host.getText(), port.getText(),
							password.getText(), username.getText());
					Proxy.socks5_Host = host.getText();
					Proxy.socks5_Port = port.getText();
					Proxy.socks5_Password = password.getText();
					Proxy.socks5_Username = username.getText();
					frame.dispose();
				}
			}
		});
